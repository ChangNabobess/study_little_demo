...
**前置条件**：钉钉凭证已配置在 `.Codex/config.json`（`access_token`、`secret`、@ 人配置）。该文件已被 `.gitignore` 忽略，不会提交。
...
> 凭证默认从 `.Codex/config.json` 读取，无需在命令行传 `access_token`/`secret`。如需临时覆盖可加 `--access_token`、`--secret`、`--userid`、`--is_at_all` 等参数。
