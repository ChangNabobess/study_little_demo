...
1. **关注点分离（Separation of Concerns）**：配置、鉴权、业务是三种不同的「关注点」，分开放各自演进、各自测试。
2. **横切关注点 + 面向切面（AOP）**：鉴权、日志、事务、异常处理——这些「每个业务都要、又不属于某个业务」的东西，用过滤器/中间件**统一处理**，而不是散在业务里。
3. **配置期 vs 请求期分离**：启动时把全局参数准备好（跑一次），请求来了只管跑业务——避免每个请求重复做初始化。
