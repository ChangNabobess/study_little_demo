...
```bash
python ".claude/skills/docx-to-md/scripts/convert.py" "<input.docx>" ["<输出.md 路径>"]
```
...
```bash
python ".claude/skills/docx-to-md/scripts/md_to_docx.py" "<input.md>" ["<输出.docx 路径>"]
```
...
- **输出位置**：默认就是「当前工作目录」。要落到项目根，就在项目根目录执行命令——这也是 Claude 在本项目的默认工作目录。
